...
export function Contact({ content }: { content: Content }) {
    ...
    async function handleSubmit(event: React.SyntheticEvent<HTMLFormElement>) {
        ...
        // Read the honeypot before any await — currentTarget is nulled after the
        // handler returns. A filled field means a bot: report success, send nothing.
        const bot = (
        ...
        if (!hasKey) {
            const subject =
                values.subject.trim() || `${values.name} — portfolio`;
                ...
                <Reveal delay={0.08}>
                    <ul className="mt-10 grid gap-px overflow-hidden rounded-xl border border-line bg-line">
                        {channels.map(({ label, value, href, Icon, copy }) => (
                            <li
                                key={label}
                                className="flex items-center gap-4 bg-bg p-5"
                            >
            ...
            <div className="min-w-0 lg:col-span-7">
                <Reveal delay={0.12}>
                    ...
                    >
                        ...
                        {/* The live region is always in the DOM — a screen reader only
                announces a change inside a region that already existed. */}
                        <div role="status" aria-live="polite">
                            {status === 'success' ? (
                                <p className="mt-5 flex items-start gap-3 rounded-lg border border-success/30 bg-success/8 px-4 py-3 text-sm text-success">
                                    <Check className="mt-0.5 shrink-0 text-base" />
                                    {contact.form.success}
                                </p>
                            ) : null}
                            {status === 'error' ? (
                                <p className="mt-5 flex items-start gap-3 rounded-lg border border-danger/30 bg-danger/8 px-4 py-3 text-sm text-danger">
                                    {contact.form.error}
                                </p>
                            ) : null}
                        </div>
